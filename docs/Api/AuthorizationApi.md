# OpenAPIAccess\Client\AuthorizationApi
...
```php
getToken($grant_type, $client_id, $client_secret, $x_request_id, $refresh_token, $username, $password): \OpenAPIAccess\Client\Model\AccessToken
```
...
$apiInstance = new OpenAPIAccess\Client\Api\AuthorizationApi(
...
[**\OpenAPIAccess\Client\Model\AccessToken**](../Model/AccessToken.md)
...
```php
revokeToken($token, $token_type_hint, $x_request_id)
```
...
try {
    $apiInstance->revokeToken($token, $token_type_hint, $x_request_id);
} catch (Exception $e) {
